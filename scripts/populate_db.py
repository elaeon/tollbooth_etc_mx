...
from tb_map_editor.model import Tollbooth, TollboothSts, TollboothStsData
...
def insert_tb_sts_from_csv():
    ldf = pl.scan_csv(file_path["tollbooth_sts"], schema=tollbooth_sts_schema)
...
def insert_tb_sts_data_from_csv(year: int):
    ldf = pl.scan_csv(file_path["tollbooth_sts_data"], schema=tollbooth_sts_data_schema)
    for i_batch, df in enumerate(ldf.collect_batches(chunk_size=100), 1):
